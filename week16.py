...
from week15 import limit_cycle_solver
...
def hopf_normal_sol(t_space, beta=1):
    x1_true = np.sqrt(1)*(np.cos(t_space))
    x2_true = np.sqrt(1)*(np.sin(t_space))
    return np.vstack((x1_true,x2_true))

def three_D_hopf(x,t=0,beta=1,sigma=-1):
    x_dot0 = beta*x[0]-x[1]+sigma*x[0]*(x[0]**2+x[1]**2)
    x_dot1 = x[0] + beta*x[1] + sigma*x[1]*(x[0]**2+x[1]**2)
    x_dot2 = -x[2]
    return np.hstack((x_dot0,x_dot1,x_dot2))

# %%

[sol, T] = limit_cycle_solver([1,2,10],hopf_normal)

#%%

#[x,t_space] = solve_to(hopf_normal,[1,2],10)
[x,t_space] = solve_to(three_D_hopf,[1,2,1],10)
x_true = hopf_normal_sol(t_space)
