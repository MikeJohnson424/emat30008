...
def VDP(x,t=None): # Define ODE: Van Der Pol Oscillator
    x1,x2 = x
    x_dot0 = x2
    x_dot1 = (1-x1**2)*x2-x1
    return np.array([x_dot0,x_dot1])

def f(x,t=None): # Define ODE: dxdt = x, solution: x(t) = x0*exp(x)
    return(x)

def g(x,t=None): # Define ODE: d2xdt2 = -x
    x1,x2 = x
    x_dot0 = x2
    x_dot1 = -x1
    return np.array([x_dot0,x_dot1])

def PPM(x,t=None,a=1,b=0.2,d=0.1): # Define predator-prey model
    x,y = x
    dx_dt = x*(1-x)-(a*x*y)/(d+x)
    dy_dt = b*y*(1-y/x)
    return np.array([dx_dt,dy_dt])

def hopf_normal_form(x, t=None, beta=1,sigma=-1): # Define hopf normal form
...
def third_order_hopf(x, t=None, beta=1, sigma=-1): # Define a third order hopf normal form ODE
...
def modified_hopf(x, t=None, beta=1):
